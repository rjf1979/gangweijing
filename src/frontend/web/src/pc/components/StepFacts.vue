<template>
  <article id="facts-step" class="task neo-panel" aria-labelledby="facts-title">
    ...
    <div class="field-block fact-editor"><label for="facts-text">职业事实</label><textarea id="facts-text" v-model="facts" rows="14" placeholder="核对并完善你的职业事实" @input="onInput"></textarea></div>
...
async function submit() {
  ...
  try {
    await api.put('/api/resume', { text: value, facts: true })
    saveDraft({ resumeText: value, facts: undefined })
    hideLoading()
    emit('next')
  } catch (err) {
    hideLoading()
...
onMounted(async () => {
  // 先以本地草稿快速填充，避免白屏闪烁；再以服务端已保存简历为准，防止旧草稿（facts 残留）串值
  facts.value = store.draft.resumeText || ''
  try {
    const resume = await api.get('/api/resume')
    if (resume.text && !userEdited.value) {
      facts.value = resume.text
      saveDraft({ resumeText: resume.text, facts: undefined })
    }
  } catch {
    // 读取失败时保留本地草稿，不阻塞页面
  }
