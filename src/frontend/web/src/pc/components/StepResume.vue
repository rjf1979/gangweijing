<template>
  <article id="resume-step" class="task neo-panel" aria-labelledby="resume-title">
    <div class="panel-heading"><span class="step-index" aria-hidden="true">01</span><div><p class="section-kicker">RESUME</p><h2 id="resume-title">上传你的简历</h2><p>支持 PDF、DOCX，也可以直接粘贴文本。</p></div></div>

    <label class="drop drop-blue" :class="{ 'has-file': file }" for="resume-file">
      <input id="resume-file" type="file" accept=".pdf,.docx" @change="onFileChange">
      <span class="drop-action">{{ file ? '已选择文件，点击可重新选择' : '选择简历文件' }}</span>
      <small id="resume-file-status">{{ file ? '已选择文件，点击可重新选择' : 'PDF 或 DOCX，文件仅用于本次分析' }}</small>
    ...
    <div class="field-block"><label for="resume-text">简历文本</label><textarea id="resume-text" v-model="text" rows="8" placeholder="没有文件时，在这里粘贴简历文本"></textarea></div>
