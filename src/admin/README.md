...
- AI 设置（AI Keys + 模型库）：
  - API Key 池：支持多套 Key 共存（官方 Key 与中转站 Key 可同时维护），每套 Key 自带名称、厂商、Base URL、启用开关，可设为「当前使用 Key」
  - 模型库：模型作为候选池统一维护（模型 ID、协议、多模态、价格等），添加模型无需再填 Base URL，地址由所选 Key 提供
  - 模型可绑定指定 Key；调用时按「模型绑定 Key > 当前使用 Key > 环境变量」优先级解析，Base URL 与 API Key 同源
...
| `OPENAI_*` / `RESEND_*` / `EMAIL_FROM` | 主服务环境变量兜底：`OPENAI_*` 仅在后台未配置 AI Key / 模型时作为 AI 调用兜底（正常应通过「AI 设置」维护）；`RESEND_*` / `EMAIL_FROM` 作为邮件配置兜底，后台数据库配置优先 |
